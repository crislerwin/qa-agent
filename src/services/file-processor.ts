...
export class FileProcessor {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      logger.log(`📁 Upload directory initialized: ${this.uploadDir}`);
    } catch (error) {
  ...
  async saveFile(file: File): Promise<FileMetadata> {
    ...
    logger.log(`✓ File saved: ${filename} (${fileType})`);
    return metadata;
  }
  ...
  async processFile(
    ...
    logger.log(
      `✓ Processed ${metadata.filename}: ${documents.length} chunks created`
    );
    return documents;
  }
  ...
  async clearAll(): Promise<number> {
    try {
      ...
      logger.log(`✓ Cleared ${count} files from upload directory`);
      return count;
    } catch (error) {
