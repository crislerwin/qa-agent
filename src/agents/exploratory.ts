...
export class ExploratoryAgent {
    ...
    async start() {
        ...
        if (this.config.sessionId) {
            const loadedState = this.sessionRepo.loadState(
        ...
        if (this.state.steps === 0) {
            ...
            if (this.config.auth?.required) {
                // Store credentials if provided in config (e.g. from CLI input)
                if (this.config.auth.credentials) {
                    ...
                    logger.log("Credentials stored/updated for this session.");
                }
                ...
                if (authRes.success) {
                    logger.log(
                        `✓ Authenticated successfully via ${authRes.method}`,
                    );
                } else {
            ...
            const lastUrl =
                this.state.history[this.state.history.length - 1]?.url;
            if (lastUrl) {
            ...
            return Array.from(elements)
                .map((el) => {
                    ...
                    if (el.id) {
                        ...
                        const href = (el as HTMLAnchorElement).getAttribute(
                            "href",
                        );
            ...
            last3Steps.every(
                (s) =>
                    ...
                    JSON.stringify(s.params) ===
                        JSON.stringify(firstOfLast3.params),
            );
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                if (isRateLimit && attempt < maxRetries) {
                    ...
                    logger.warn(
                        `Rate limit handling: Waiting ${waitTime}ms...`,
                    );
    ...
    private async executeAction(
        ...
        try {
            switch (action) {
                ...
                case "click":
                    let selector =
                        typeof params === "string" ? params : params?.selector;
                    if (!selector) {
                        throw new Error(
                ...
                case "fill_form":
                    ...
                    logger.log(
                        `Filling form: ${params.selector} = ${params.value}`,
                    );
                ...
                case "find_broken_images":
                    ...
                    if (findings.length > 0) {
                        brokenImgScreenshot =
                            await this.takeScreenshot("broken-images");
                    }
