...
import { CredentialProvider, type AuthConfig as CredConfig, type Credentials } from "./credential-provider.js";
    ...
    async authenticate(
        ...
        try {
            ...
            if (!loginFlow) {
                ...
                // For now, fail.
                return { success: false, method: "detection-failed", error: "Could not detect login flow" };
            }

            // 4. Get credentials
            let credentials;
            try {
                credentials = await this.credentialProvider.getCredentials(appIdentifier);
            } catch (e) {
                return { success: false, method: "credential-retrieval", error: (e as Error).message };
            }
