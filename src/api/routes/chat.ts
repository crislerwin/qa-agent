  ...
  .post(
    ...
    async ({ body }) => {
      ...
      try {
        ...
        logger.info("Creating conversational agent with default model...");
        const agent = createConversationalAgent({
          model: getDefaultModel(),
          systemPrompt: CHAT_SYSTEM_PROMPTS[locale],
        });
        logger.info("Conversational agent created successfully");
  ...
  .post(
    ...
    async ({ body }) => {
      ...
      try {
        ...
        // Create agent with default model from environment
        logger.info("Creating web agent with default model...");
        ...
        // Invoke agent
        logger.info("Invoking web agent...");
        const response = await agent.invoke({
          messages,
        });
        logger.info("Web agent invoked successfully");
