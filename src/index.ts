  ...
  if (enableTestGeneration) {
    const testExecutionMode = await clack.select({
      ...
      options: [
        ...
        {
          value: "parallel",
          label: "Parallel (Execute multiple tests concurrently)",
        },
      ],
    });
...
${wrapText(result.action, 80)}`,
      ...
      if (enableTestGeneration) {
        explorationOptions.push({
          value: "generate-tests",
          label: "Generate Tests Now (Continue after)",
        });
      }
      ...
      if (answer === "stop") {
        ...
        try {
          ...
          if (generatedTests.length > 0) {
            ...
            clack.log.success(
              `✅ Generated ${generatedTests.length} automated tests from current findings`,
            );
            ...
            const summary = {
              ...
              medium: generatedTests.filter((t) => t.priority === "medium")
                .length,
              low: generatedTests.filter((t) => t.priority === "low").length,
            };

            clack.log.info(
              `📊 Generated ${summary.total} E2E tests (${summary.high} high, ${summary.medium} medium, ${summary.low} low priority)`,
            );
          } else {
    ...
    if (enableTestGeneration) {
      ...
      try {
        ...
        if (generatedTests.length > 0) {
          testSpinner.stop(
            `Generated ${generatedTests.length} automated tests`,
          );

          // Show test summary
          const testSummary = generatedTests.reduce(
            (acc, test) => {
...
📄 Execution report saved to: ./reports/`,
            "Tests Generated",
          );
        } else {
          testSpinner.stop(
            "No tests generated (no findings or test generation disabled)",
          );
        }
      } catch (error) {
        testSpinner.stop("Test generation failed");
        logger.error("Test generation failed:", error);
        clack.log.warn(
          "Test generation encountered an error, but exploration was successful.",
        );
      }
    }
  } catch (error: any) {
