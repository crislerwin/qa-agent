...
describe("Database Consolidation", () => {
    ...
    test("CredentialStorage should list stored credentials", async () => {
        const credStorage = new CredentialStorage(database);

        await credStorage.set("app1", { password: "p1" });
        await credStorage.set("app2", { password: "p2" });

        const list = await credStorage.list();
